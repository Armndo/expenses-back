...
class ExpenseController extends Controller
{
    public function index(Request $request) {
        ...
        $expenses = $user->sources()->with(["expenses" => fn(Builder $query,) =>
            ...
            ->whereRaw("date between date(date_trunc('month', '$start'::date)::date + sources.cutoff) and date(date_trunc('month', '$start'::date)::date - 1 + sources.cutoff + interval '1 month')")
        ...
        $instalments = $user->sources()->with(["expenses" => fn(Builder $query) =>
            $query->select("expenses.*")
            ->join("sources", "sources.id", "expenses.source_id")
            ->whereRaw("date(\"date\" + interval '1 month' * (\"instalments\" - 1)) >= date(date_trunc('month', '$start'::date)::date + sources.cutoff)")
            ->whereRaw("\"date\" <= date(date_trunc('month', '$start'::date)::date - 1 + sources.cutoff + interval '1 month')")
